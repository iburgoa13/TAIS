...
bool posCorrecta(int i, int j, int alto, int ancho) {
...
void ejercicio(std::vector<std::vector<bool>>&marked, int ancho, int alto, std::pair<int,int>ini, std::pair<int,int> fin,
	...
	while (!q.empty()) {
		...
		for (auto dir : direcciones) {
			...
			if(posCorrecta(ni,nj,alto,ancho) && !marked[ni][nj]){
...
void colocaSensor(std::queue<tSensores> & sensor, std::vector<std::vector<bool>> &marked, std::vector<std::vector<char>>&mapa, int ancho, int alto){// Matriz<bool>&marked, Matriz<char>&mapa) {
	while (!sensor.empty()) {
		...
		marked[x][y] = true;
		mapa[x][y] = '+';
		while (i < aux.valor && posCorrecta(x - 1, y,alto,ancho) && mapa[x - 1][y] != '#') {
			marked[x - 1][y] = true;
			mapa[x - 1][y] = '+';
			x--;
			i++;
		...
		x = aux.x;
		int j = 0;
		while (j < aux.valor && posCorrecta(x, y - 1,alto,ancho) && mapa[x][y - 1] != '#') {
			marked[x][y - 1] = true;
			mapa[x][y - 1] = '+';
			y--; j++;
		}
...
void resuelveCaso() {
	...
	for (int i = 0; i < n; i++) {
		std::cin >> ancho >> alto;
		std::vector<std::vector<char>> mapa(alto, std::vector<char>(ancho));
		//Matriz<bool>marked(alto,ancho,false);
		std::vector<std::vector<bool>> marked(alto, std::vector<bool>(ancho));
		...
		colocaSensor(sensores, marked,mapa,ancho,alto);
